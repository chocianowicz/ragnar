...
def test_set_chunker_changes_chunking_for_next_ingest(tmp_path):
    ...
    pipeline = Pipeline(parser, FixedChunker(target_chars=1000), FakeEmbedder(), store)

    path = tmp_path / "a.pdf"
    path.write_bytes(b"data")
    first = pipeline.ingest(path, "d1")

    pipeline.set_chunker(FixedChunker(target_chars=50, overlap_chars=0))
...
def test_set_chunker_does_not_affect_already_ingested_chunks():
    ...
    pipeline.set_chunker(FixedChunker(target_chars=1))
