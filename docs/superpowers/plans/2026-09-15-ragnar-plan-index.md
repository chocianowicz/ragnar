...
| 1 | `phase-1-structure.md` *(to write)* | Extract `answer_job` → `generation/answering.py`; promote test doubles to `tests/fakes.py`; dedupe `_pool`/`_widen`; `Chunk.key()`; `dataclasses.asdict` traces; publish at citation build; `RetrievalRequest` dataclass; split `ui/app.py`; fragment polling; `migrate.py` tests | Phase 0 merged |
