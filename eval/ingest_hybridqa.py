...
def main() -> None:
    ...
    parser.add_argument("--collection", default="hybridqa_structural")
    args = parser.parse_args()

    # Production chunking (config.yaml), so the benchmark measures the
    # chunks users actually get. A report's provenance names the git
    # revision, which pins config.yaml too.
    cfg = Config()
    chunking = cfg.chunking
    chunker = StructuralChunker(
        target_tokens=int(chunking.get("target_tokens", 500)),
        overlap_tokens=int(chunking.get("overlap_tokens", 50)),
        rows_per_group=int(chunking.get("table_rows_per_group", 20)))
    ...
    for table_id, data in tables.items():
        ...
        pieces = chunk_table_markdown(
            md, rows_per_group=chunker.rows_per_group,
            max_chars=chunker.target_chars) or [md]
        ...
        for path, text in data["passages"].items():
            ...
            for i, piece in enumerate(chunker._split(text)):
                chunks.append(Chunk(
