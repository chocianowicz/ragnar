...
class FixedChunker:
    ...
    Deliberately simpler than StructuralChunker: it does not emit table
    aggregate summaries, inject sheet names into table text, or set
    is_summary. A `fixed` vs `structural` eval comparison is therefore only
    apples-to-apples on prose-only corpora, not table-heavy ones.
    """
