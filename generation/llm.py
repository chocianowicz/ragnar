...
class OllamaLLM:
    ...
    def _payload(self, system: str, user: str, stream: bool,
        ...
        if history:
            # Prior turns, already windowed and stripped of app-only keys
            # by the caller. They go in as real chat turns rather than
            # pasted into the user message, so the model treats them as
